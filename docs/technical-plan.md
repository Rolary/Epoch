...
- 四种首页养料分别提交 `absorb_crystal`、`absorb_spark`、`absorb_droplet`、`absorb_pulse`，服务端决定对应材料变化；旧 `catalyze` 动作继续保留兼容。
...
- `game-core` 的主动节奏回归以约 9 秒一次养料、45 秒一次潮汐收获进行代表性模拟，约束第一项演化、原初细胞、第一章完成和第二章完成的最长时间，并验证第一次材料交换是滤食角色的前置见证。
...
第三章的设计基线见 `docs/third-chapter-progression.md`，阶段执行顺序见 `docs/third-chapter-build-plan.md`。当前已实现共享章节/栖位类型、第三章见证与派生进度、水线显现、湿岸附着、水分分栖、退潮事件、首次回流和“盐晶爬上湿岩”事件，并提供非生产环境 `POST /debug/third-chapter-save` 的 `exposed / shore / niches / event / exchange / salt / salted` 切片；开发模式可用 `?debugChapter3=salt` 等查询参数进行非持久化视觉验收。其余技术边界保持如下：
...
第三章继续按“共享类型和归一化 -> 核心规则和测试 -> Phaser 水线反馈 -> React 主线与生命史 -> 浏览器和移动端验收”的顺序推进。首条体验切片、`niche_split` 与首个扩展岸线事件已完成，后续仍需补充骤雨冲岸、分支平衡、章节总结与完整发布验收，不把当前扩展误标为完整章节完成。
